...
runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === 'NT_CLEAR_CONTEXT_HARD') {
    Promise.resolve()
      .then(() => clearContextHard())
      .then(() => {
        if (typeof sendResponse === 'function') {
      ...
      .catch((error) => {
        console.warn('Failed to hard reset context.', error);
        if (typeof sendResponse === 'function') {
...
async function clearContextCacheForUrl(url) {
  if (!url) return;
  const store = await new Promise((resolve) => {
    chrome.storage.local.get([CONTEXT_CACHE_KEY], (data) => {
      resolve(data?.[CONTEXT_CACHE_KEY] || {});
  ...
  try {
    ...
    appendDebugEvent(CALL_TAGS.STORAGE_DROPPED, error?.message || 'context-cache-clear-failed');
  }
}

async function clearStoredObjectByKey(storageKey) {
  const clearInArea = (area) =>
    new Promise((resolve) => {
      if (!area?.set) {
        resolve(false);
        return;
      }
      area.set({ [storageKey]: {} }, () => resolve(true));
    });
  try {
    await clearInArea(chrome?.storage?.session);
  } catch (error) {
    // ignore session errors
  }
  try {
    await clearInArea(chrome?.storage?.local);
  } catch (error) {
    // ignore local errors
  }
}

async function clearContextHard() {
  const url = location.href;
  const settings = await requestSettings();
  const contextEnabled =
    typeof settings?.contextGenerationEnabled === 'boolean'
      ? settings.contextGenerationEnabled
      : debugState
        ? debugState.contextFullStatus !== 'disabled'
        : true;
  const nextStatus = contextEnabled ? 'pending' : 'disabled';
  ...
  if (debugState) {
    debugState.context = '';
    debugState.contextStatus = nextStatus;
    debugState.contextFull = '';
    debugState.contextFullStatus = nextStatus;
    debugState.contextShort = '';
    debugState.contextShortStatus = nextStatus;
    debugState.contextFullRefId = '';
    debugState.contextShortRefId = '';
    debugState.contextFullTruncated = false;
    debugState.contextShortTruncated = false;
    schedulePersistDebugState('hard-reset-context');
  } else if (url) {
    const existing = await getTranslationDebugObject();
    const entry = existing[url];
    if (entry) {
      existing[url] = {
        ...
        contextFull: '',
        contextFullStatus: nextStatus,
        contextShort: '',
        contextShortStatus: nextStatus,
        contextFullRefId: '',
        contextShortRefId: '',
  ...
  await clearContextCacheForUrl(url);
  await clearStoredObjectByKey('translationShortContextByBlock');
  await clearStoredObjectByKey('manualTranslateOutputsByBlock');
}
